    ...
    <script>
      $(document).ready(function(){
            $("#submit").click(function(){
                ...
                var pid = $('#pid').val();
                $.post("/AO/index.php/Home/Project/newProject",{'pid':pid,'content':content},function(data){
                    if(data == 0)
                        alert("新建/修改失败");
                    else
                    {
                        alert("新建/修改成功");
                        window.location.href="/AO/index.php/Home/Project/index";
                    }  
                });
            });
            ...
            <div class="col-md-8 col-md-offset-2" style="border: 2px solid #ddd; border-radius: 10px; padding-left: 3%;">
              <table class="table table-hover">
                ...
                <?php if(is_array($project)): foreach($project as $key=>$val): ?><tr>
                  <td id="<?php echo ($val["pid"]); ?>"><a href="/AO/index.php/Home/Project/subProject?pid=<?php echo ($val["pid"]); ?>" target="_blank"><?php echo ($val["content"]); ?></a></td>
                  <td><?php echo ($val["username"]); ?></td>
                  <td><?php echo (date("Y-m-d H:i:s",$val["date"])); ?></td>
                   <?php if($pkid == 5): ?><td><button class="btn btn-default modify" data-toggle="modal" value="<?php echo ($val["pid"]); ?>" data-target="#myModal">修改</button>
                  </td><?php endif; ?>
